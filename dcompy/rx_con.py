...
def rx_con(port: int, src_ia: int, frame: Frame):
    """
    接收到连接帧时处理函数
    """
    resp, err = service_callback(frame.control_word.rid, frame.payload)
    ...
    if err != SYSTEM_OK:
        send_rst_frame(port, src_ia, err, frame.control_word.rid, frame.control_word.token)
        return

    if resp and len(resp) > SINGLE_FRAME_SIZE_MAX:
        # 长度过长启动块传输
        block_tx(port, src_ia, CODE_ACK, frame.control_word.rid, frame.control_word.token, resp)
        return
