...
def _deal_recv(port: int, src_ia: int, frame: Frame):
    if frame.control_word.code == CODE_CON or frame.control_word.code == CODE_NON:
        rx_con(port, src_ia, frame)
        return
    if frame.control_word.code == CODE_ACK:
        rx_ack_frame(port, src_ia, frame)
        return
    if frame.control_word.code == CODE_BACK:
        block_rx_back_frame(port, src_ia, frame)
        return
    if frame.control_word.code == CODE_RST:
        if not frame.payload or len(frame.payload) != 1 or frame.control_word.payload_len != 1:
            return
        rx_rst_frame(port, src_ia, frame)
        block_rx_deal_rst_frame(port, src_ia, frame)
        block_tx_deal_rst_frame(port, src_ia, frame)
        return


def receive(port: int, src_ia: int, data: bytearray):
    """
    ...
    if frame.control_word.block_flag == 0:
        _deal_recv(port, src_ia, frame)
    else:
        block_frame, err = bytes_to_block_frame(data)
        if not err:
            return
        block_rx_receive(port, src_ia, block_frame)
