...
def _block_tx_send_frame(item: _Item, offset: int):
    ...
    block_send(item.port, item.dst_ia, frame)


def block_tx(port: int, dst_ia: int, code: int, rid: int, token: int, data: bytearray):
    """
    块传输发送
    """
    ...
    if _is_item_exist(port, dst_ia, code, rid, token):
        _lock.release()
        return

    item = _create_item(port, dst_ia, code, rid, token, data)
    _block_tx_send_frame(item, 0)
    item.first_frame_retry_num += 1
...
def _is_item_exist(port: int, dst_ia: int, code: int, rid: int, token: int) -> bool:
    for item in _items:
        if item.port == port and item.dst_ia == dst_ia and item.code == code and item.rid == rid \
                and item.token == token:
            return True
    return False


def _create_item(port: int, dst_ia: int, code: int, rid: int, token: int, data: bytearray) -> _Item:
    item = _Item()
    item.port = port
    item.dst_ia = dst_ia
    item.code = code
...
def block_rx_back_frame(port: int, src_ia: int, frame: Frame):
    """
    接收到BACK帧时处理函数
    """
    ...
    _lock.acquire()
    for item in _items:
        if _check_item_and_deal_back_frame(port, src_ia, frame, item):
            break
    _lock.release()


def _check_item_and_deal_back_frame(port: int, src_ia: int, frame: Frame, item: _Item) -> bool:
    """
...
def block_tx_deal_rst_frame(port: int, src_ia: int, frame: Frame):
    """
    块传输发送模块处理复位连接帧
    """
    _lock.acquire()
    for item in _items:
        if item.port == port and item.dst_ia == src_ia and item.rid == frame.control_word.rid and \
                item.token == frame.control_word.token:
            _items.remove(item)
            break
    _lock.release()
